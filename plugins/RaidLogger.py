...
class RaidLogger(SWPlugin.SWPlugin):
    ...
    def process_request(self, req_json, resp_json):
        ...
        if command == 'BattleRiftOfWorldsRaidStart':
            ...
            config['stage'] = '%s R%s' % (identify_raid(resp_json['battle_info']['room_info']['raid_id']), resp_json['battle_info']['stage_id'])
            config['wizard'] = resp_json['wizard_info']['wizard_id']
            team = []
            for i in resp_json['battle_info']['user_list']:
                if i['wizard_id'] != resp_json['wizard_info']['wizard_id']: team.append(i['wizard_name']) 
            config['team'] = team
    ...
    def log_end_battle(self, req_json, resp_json, config):
        ...
        log_entry = "%s,%s,%s,%s,%s,%s," % (time.strftime("%Y-%m-%d %H:%M"), config['stage'] if 'stage' in config else 'unknown', win_lost, elapsed_time, config['team'][0], config['team'][1])

        if win_lost == 'Win':
            order = 0
            for i in resp_json['battle_reward_list']:
                if config['wizard'] != i['wizard_id']: order += 1 
                else: break
        ...
        filename = config['log_raids_filename']
