...
class SummonLogger(SWPlugin):
    ...
    def log_summon(self, req_json, resp_json, config):
        ...
        with open(filename, "ab") as log_file:
            field_names = ['wizard_id', 'server_tz', 'date', 'scroll', 'unit_name', 'attribute', 'grade', 'awake']

            header = {'wizard_id': 'wizard id', 'server_tz': 'server timezone',
                      'date': 'Date', 'scroll': 'Scroll', 'unit_name': 'Unit', 'attribute': 'Attribute',
                      'grade': 'Grade',
                      'awake': 'Awakened'}

            excludes = ['wizard_id', 'server_tz']

            SWPlugin.call_plugins('process_csv_row', ('summon_logger', 'header', (field_names, header, excludes)))
